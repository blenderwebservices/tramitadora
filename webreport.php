...
$(document).ready(function(){
	...
	if(!count($arr_UserGroups))
	{
		$b_includes .= '$("#admin_sql").click(function(){
			...
			$("#alert")
				.html("'.mlang_message("PASSWORD").':&nbsp;<input type=password id=admin_password size=30 value=\"\">")
				.dialog("option", "buttons", {
					"'.mlang_message("CANCEL").'": function() { $(this).dialog("close"); },
					"'.mlang_message("OK").'": function() {
						$.ajax({
							type: "POST",
							...
							success: function(msg){
								if ( msg == "OK" ) {
									...
									$("#alert").dialog("option", "buttons", {"'.mlang_message("CANCEL").'": function() { $(this).dialog("close");}});
									$("#alert").html("<p>'.mlang_message("INVALID_PASSWORD").'</p>").dialog("open");
								}
							}
						});				
					}
...
if($_SESSION["UserID"]!="Guest")
	$strLogin.="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class=tablelinks href=\"".GetTableLink("changepwd")."\">".mlang_message("CHANGE_PASSWORD")."</a>";
