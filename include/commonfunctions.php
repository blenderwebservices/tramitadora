...
function getLangFileName($langName)
{
	...
	$langArr["Portuguese(Standard)"] = "Portugal";
...
function checkTableName($shortTName )
{
	...
	if ("registro" == $shortTName )
		return true;
	if ("users" == $shortTName )
		return true;
	if ("tiposdoc" == $shortTName )
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("registro");
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("tiposdoc");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function GetUserPermissionsStatic( $table )
{
	...
	if( $table=="registro" )
	{
		if( $sUserGroup=="editor" )
		{
			return "AES".$extraPerm;
		}
		if( $sUserGroup=="captura" )
		{
			return "AS".$extraPerm;
		}
		if( $sUserGroup=="admin" )
...
function SetLangVars($xt, $prefix, $pageName = "", $extraparams = "")
{
	...
	$xt->assign("DutchLANGLINK", "Dutch" != $currentLang);
	...
	$xt->assign("Portuguese(Standard)LANGLINK", "Portuguese(Standard)" != $currentLang);
...
function getDefaultLanguage()
{
	if( strlen($_SESSION["language"]) == 0 && $_SERVER['HTTP_ACCEPT_LANGUAGE'] )
	{
		...
		$arrWizardLang[] = "Portuguese(Standard)";
