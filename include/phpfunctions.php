...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="registro" && $field=="fecha")
	{
		$fechaToFormat = new DateTime($data['fecha']);
$value = $fechaToFormat->format("ymd.Hi.w");;
		return $value;
	}
				if($table=="registro" && $field=="texto")
	{
		$value= "<pre style='overflow:visible;'>".$data["texto"]."</pre>";
		return $value;
	}
				if($table=="tiposdoc" && $field=="template")
	{
		$value= "<pre style='overflow:visible;'>".$data["template"]."</pre>";
		return $value;
	}
	return $value;
}
...
{
	$value = "";
				if($table=="registro" && $field=="documents_fs")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="users" && $field=="groupid")
	{
		return "captura";
	}
				if($table=="users" && $field=="active")
	{
		return 1;
	}
	return "";
